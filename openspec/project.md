...
- 存储和缓存：D1 存业务数据和后台配置，R2 存放头像和未来媒体附件；KV 仅可用于非关键短期缓存，不承载后台业务配置。
- 异步任务：Cloudflare Queues 用于 AI 生成瓶子、AI 回复和日志写入。
- AI：建立供应商无关的适配层，后续可接入多平台模型 API。

## 依赖版本策略

- 用户端 `apps/client` 按 uni-app Vue3 发行线维护，Vue、Vite、`@dcloudio/*`、`@dcloudio/types` 必须成组核对和升级。
- 升级用户端 Vue 或 Vite 前，先查询 `@dcloudio/uni-app` 的 `vue3` dist-tag，以及对应 `@dcloudio/vite-plugin-uni` 的 peer 依赖；不得按管理后台或普通 Vue 项目的最新版单独升级。
- 管理后台 `apps/admin` 是独立 Vue/Vite 应用，可以按 Vue、Vite、Element Plus 的稳定版本线升级，但不得反推用户端版本。
- 当前用户端基线：`@dcloudio/* 3.0.0-5010420260703001`、`vue 3.4.21`、`vite 5.2.8`、`@dcloudio/types 3.4.31`。
...
6. 实现完成后，先运行 `pnpm spec:validate` 和相关代码校验，并把 `tasks.md` 全部勾选完成。
7. 使用 `pnpm spec:status` 查看已完成但仍处于 active 的变更。
8. 使用 `pnpm spec:archive <change-id>` 归档单个已完成变更，让 OpenSpec CLI 合并主规格并移走变更目录。
9. 纯文档、部署脚本或不需要合并规格的变更，使用 `pnpm spec:archive <change-id> --skip-specs`。
10. 归档后再次运行 `pnpm spec:validate`，确认 active 变更和主规格都通过校验。
