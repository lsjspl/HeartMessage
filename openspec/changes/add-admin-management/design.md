...
第一阶段使用系统敏感配置提供首次超级管理员账号密码：

- `ADMIN_USERNAME`
- `ADMIN_PASSWORD`

敏感配置保存在 `CONFIG_KV` 的独立 key 中，后台列表只返回配置状态、来源和脱敏预览，不返回完整明文。本地开发允许使用显式本地默认值；生产地址不得使用本地默认账号完成首次初始化。
...
密钥、token 签名密钥、微信配置和 AI Key 等敏感值通过后台敏感配置接口写入 `CONFIG_KV`，不得继续通过 `wrangler vars` 配置常用业务参数。日志只记录配置键和更新动作，不记录配置值。
